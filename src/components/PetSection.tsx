...
export const PetSection: React.FC<PetSectionProps> = ({
  ...
  const getSegmentedBar = (val: number, max: number = 100, color: string = 'bg-[#4caf50]') => {
    ...
    return (
      <div className="flex gap-1.5 w-full bg-[#1e1e1e] border-2 border-black p-1">
  ...
  return (
    <div className="bg-[#3d3d3d] pixel-border p-5 text-white flex flex-col justify-between h-full">
      ...
      <div>
            ...
            <button
              ...
              className={`flex-1 font-press-start text-[10px] py-2 border-2 ${
                activeTab === tab
                  ...
                  : 'bg-[#252525] text-gray-400 border-black hover:text-white'
        ...
        {activeTab === 'status' && (
          <div className="flex flex-col gap-4">
            {/* NAME EDIT */}
            <div className="flex justify-between items-center gap-2 border-b-2 border-black pb-2">
              {isEditingName ? (
                <div className="flex gap-1 w-full">
                  <input
                    ...
                    maxLength={14}
                    className="flex-1 bg-[#1e1e1e] border-2 border-black font-press-start text-xs p-1 text-white focus:outline-none"
                  />
                  <button onClick={handleRename} className="bg-[#4caf50] border-2 border-black p-1 text-black">
                ...
                <div className="flex justify-between items-center w-full">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-vt323 text-lg text-gray-400">Lv.{pet.level}</span>
                  </div>
                  <button onClick={() => setIsEditingName(true)} className="text-gray-450 hover:text-white">
            ...
            <div className="flex bg-[#252525] border-4 border-black p-4 items-center justify-center relative min-h-[170px]">
              {/* Sprite renderer */}
            ...
            {/* EXP BAR */}
            <div>
              <div className="flex justify-between text-[11px] font-press-start text-gray-300 mb-1">
                <span>XP PROGRESS</span>
                <span>{pet.exp}/{pet.maxExp}</span>
              </div>
              <div className="h-6 w-full bg-[#1e1e1e] border-2 border-black p-0.5">
                <div 
        ...
        {activeTab === 'shop' && (
          <div className="flex flex-col gap-3 max-h-[380px] overflow-y-auto pr-1">
            <p className="font-vt323 text-sm text-gray-400 mb-1">Gunakan Koin Emas (G) hasil menyelesaikan tugas untuk merawat peliharaanmu!</p>
            {shopItems.map((item) => {
              ...
              return (
                <div key={item.id} className="bg-[#292929] border-2 border-black p-2 flex justify-between items-center gap-2">
                  <div className="text-2xl">{item.icon}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className="font-press-start text-[10px] text-[#ffeb3b] truncate uppercase">{item.name}</span>
                    </div>
                    <span className="font-vt323 text-xs text-gray-400 block leading-tight">{item.description}</span>
                  </div>
                  <div className="flex flex-col items-end gap-1.5">
                    ...
                    <button
                      ...
                      className={`font-press-start text-[9px] py-1 px-2 border-2 ${
                        disabled
                          ? 'bg-[#444] border-[#666] text-gray-500 cursor-not-allowed'
                          : 'bg-[#4caf50] border-black text-black hover:bg-[#81c784]'
                      }`}
                      style={{ imageRendering: 'pixelated' }}
                    >
                      BELI
        ...
        {activeTab === 'habitat' && (
          <div className="flex flex-col gap-4">
            <p className="font-vt323 text-sm text-gray-400">Pilih spesies peliharaan favoritmu, atau ubah spesies peliharaan saat ini.</p>
              ...
              {(['slime', 'cat', 'dragon', 'dog', 'panda', 'chicken'] as PetSpecies[]).map((sp) => {
                ...
                return (
                  <button
                    ...
                    className={`bg-[#2c2c2c] border-4 p-2 flex flex-col items-center gap-2 transition-transform cursor-pointer ${
                      isActive ? 'border-[#ffeb3b] scale-105' : 'border-black hover:border-gray-500'
                    }`}
                  >
            ...
            <div className="bg-[#292929] border-2 border-dashed border-black p-3 rounded-none text-center mt-2">
              <span className="font-press-start text-[9px] text-[#ffeb3b] uppercase block mb-1">INFO KELAS</span>
              <p className="font-vt323 text-xs text-gray-300">
      ...
      {activeTab === 'status' && pet.status !== 'dead' && (
        <div className="flex gap-2 mt-4 pt-4 border-t-2 border-black">
          <button
            ...
            className="flex-1 bg-pink-500 hover:bg-pink-400 font-press-start text-[10px] text-white py-2 border-4 border-black shadow-[4px_4px_0px_#000000] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all disabled:opacity-50"
          >
            MAIN
          </button>
          <button
            onClick={() => onInteract(pet.status === 'sleeping' ? 'wake' : 'sleep')}
            className="flex-1 bg-indigo-600 hover:bg-indigo-505 font-press-start text-[10px] text-white py-2 border-4 border-black shadow-[4px_4px_0px_#000000] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all"
          >
      ...
      {/* DEATH ACTION BAR */}
      {pet.status === 'dead' && (
        <div className="mt-4 p-3 bg-red-900 border-4 border-black text-center animate-pulse">
          <span className="font-press-start text-xs block text-white mb-2">RIP {pet.name.toUpperCase()}</span>
          <button
            onClick={() => onInteract('revive')}
            disabled={coins < 100}
            className="w-full bg-[#ffeb3b] text-black border-2 border-black p-1 font-press-start text-[9px] hover:bg-yellow-400 disabled:opacity-50"
          >
             REVIVE POTION (100 G)
          </button>
        </div>
